...
func startServer(hostAddr string, serviceType string, blockStoreAddrs []string) error {
	...
	if serviceType == "meta" {
		metaSrv := surfstore.NewMetaStore(blockStoreAddrs)
		surfstore.RegisterMetaStoreServer(grpcServer, metaSrv)
	} else if serviceType == "block" {
		blockSrv := surfstore.NewBlockStore()
		surfstore.RegisterBlockStoreServer(grpcServer, blockSrv)
	} else {
		log.Printf("both")
		metaSrv := surfstore.NewMetaStore(blockStoreAddrs)
		surfstore.RegisterMetaStoreServer(grpcServer, metaSrv)
	...
	listner, err := net.Listen("tcp", hostAddr)

	if err != nil {
		return err
	}
	log.Println("running surfStore on:", hostAddr)
